...
from vllm.utils import length_from_prompt_token_ids_or_embeds, compress_and_encode
    ...
    def __init__(
        ...
        self.stats = (
            RequestStateStats(
                ...
                api_server_arrival_time=metrics.get("api_server_arrival_time", 0.0),
                process_input_finish_time=metrics.get("process_input_finish_time", 0.0),
                mm_load_start_ts=metrics.get("mm_load_start_ts", 0.0),
                mm_load_end_ts=metrics.get("mm_load_end_ts", 0.0),
            )
            if log_stats
            else None
        )
...
class OutputProcessor:
    ...
    def process_outputs(
        ...
        for engine_core_output in engine_core_outputs:
            ...
            if request_output := req_state.make_request_output(
                ...
                if (
                    req_state.output_kind == RequestOutputKind.DELTA
                    and len(request_output.outputs) == 1
                ):
            ...
            if req_state.observable_context:
                req_state.observable_context.update_from_output(engine_core_output, delta_text)
            ...
            if finish_reason is not None:
                ...
                self._update_stats_from_finished(req_state, finish_reason,
                                                 iteration_stats)
                if req_state.stats:
                    finished_stats = req_state.stats.finished_stats
                    if request_output:
                        request_output.finished_stats = finished_stats
                    elif req_state.queue is not None:
                        req_state.queue.output.finished_stats = finished_stats
                if self.tracer:
                    self.do_tracing(req_state=req_state, engine_core_output=engine_core_output, iteration_stats=iteration_stats)
            elif req_state.stats:
    ...
    def do_tracing(self, 
        ...
        trace_headers = engine_core_output.trace_headers if engine_core_output else req_state.trace_headers
        ...
        with (self.tracer.start_as_current_span(
            ...
            finish_reasons = [str(engine_core_output.finish_reason if engine_core_output else FinishReason.ABORT)]
            span.set_attribute(
            ...
            if req_state.stats:
                span.set_attribute(
                    SpanAttributes.ALIPAY_REQ_METRIC,
                    msgspec.json.encode(req_state.stats).decode('utf-8')
                )
