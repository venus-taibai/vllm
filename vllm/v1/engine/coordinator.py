        ...
        with make_zmq_socket(
            ...
            while True:
                ...
                if publish_front in events:
                    ...
                    logger.debug("Receive msg from publish_front, engine_index = %d, wave = %d, current_wave = %d, engines_running = %d", engine_index , wave, self.current_wave, self.engines_running)
                ...
                if output_back in events:
                    ...
                    logger.debug("Receive msg from output_back, outputs = %s, current_wave = %d, engines_running = %d", outputs, self.current_wave, self.engines_running)
                    ...
                    if outputs.wave_complete is not None:
                        ...
                        self.engines_running = True
                        self.stats_changed = True
                        self._send_start_wave(publish_back, self.current_wave, eng_index)

    @staticmethod
    def _send_start_wave(socket: zmq.Socket, wave: int,
                         exclude_engine_index: Optional[int]):
        logger.debug("send start wave, wave = %d, exclude_engine_index = %d", wave, exclude_engine_index)
