...
class ObservableContext:
    ...
    scheduled_time: list[int]
    token_time: list[int]
    candidate_token_ids: Optional[list[Union[int, list[int]]]]
    candidate_decoded_tokens: Optional[list[Union[str, list[str]]]]
    ...
    def _update_iter_stats(
        self, iter_stats: IterStats, new_token_ids: list[int], delta_text: str
    ) -> None:
        ...
        for index in range(new_tokens_num):
        ...
        if iter_stats.logprobs_tensors_for_trace:
            token_ids_lst, logprobs_lst, ranks_lst = (
                iter_stats.logprobs_tensors_for_trace
            )
            for _, logprobs, token_ids in zip(ranks_lst, logprobs_lst, token_ids_lst):
                ...
                decoded_tokens = (
                    ...
                    else (convert_ids_list_to_tokens(self.tokenizer, token_ids, True))
                )
    ...
    def update_from_output(self, output: EngineCoreOutput, delta_text: str) -> None:
